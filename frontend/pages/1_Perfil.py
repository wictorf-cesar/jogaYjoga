...
def tela_atleta():
    ...
    ATLETA = {
        "nome": "João Silva",
        "membro_desde": "março de 2024",
        "esportes": ["⚽ Futebol", "🏐 Vôlei", "🎾 Tênis"],
        "quadra_fav": "Arena Boa Vista",
        "nivel": "Intermediário",
    }
    KPI = {
        "partidas_mes": 12,
    ...
    historico = pd.DataFrame(
        {
            "Semana": semanas * 3,
            "Esporte": ["Futebol"] * 8 + ["Vôlei"] * 8 + ["Tênis"] * 8,
            "Partidas": [random.randint(0, 4) for _ in range(24)],
        }
    )
    ...
    gastos = pd.DataFrame(
        {
            "Mês": meses * 3,
            "Esporte": ["Futebol"] * 6 + ["Vôlei"] * 6 + ["Tênis"] * 6,
            "Gasto": [random.randint(80, 200) for _ in range(18)],
        }
    )
    hoje = date.today()
    proximas_reservas = [
        {
            "quadra": "Arena Boa Vista",
            "esporte": "⚽ Futebol",
    ...
    historico_partidas = pd.DataFrame(
        {
            "Data": [
                (hoje - timedelta(days=d)).strftime("%d/%m")
                for d in [2, 4, 6, 9, 11, 14]
            ],
            "Quadra": [
                "Arena Boa Vista",
                "Quadra do Derby",
    ...
    # ── RENDERIZAÇÃO ATLETA ──
    tags_html = "".join(
        f"<span class='sport-tag'>{e}</span>" for e in ATLETA["esportes"]
    )
    st.markdown(
        f"""
    <div class="hero">
    ...
    with col_ativ:
        st.markdown(
            '<div class="sec-title">📈 Partidas por Semana</div>',
            unsafe_allow_html=True,
        )
        fig_hist = px.bar(
            historico,
            ...
            color="Esporte",
            barmode="group",
            color_discrete_map=CORES_ESPORTE,
        )
    ...
    with col_g1:
        st.markdown(
            '<div class="sec-title">💸 Gastos Mensais por Esporte</div>',
            unsafe_allow_html=True,
        )
        fig_gas = px.line(
            gastos,
        ...
        fig_gas.update_layout(
            ...
            legend=dict(font=dict(color="#a0aec0")),
            xaxis=dict(gridcolor="#2a2a4a"),
            yaxis=dict(gridcolor="#2a2a4a", tickprefix="R$ "),
        )
        st.plotly_chart(fig_gas, use_container_width=True)

    with col_g2:
        st.markdown(
            '<div class="sec-title">🏅 Tempo por Esporte</div>', unsafe_allow_html=True
        )
        df_tempo = pd.DataFrame(
            {"Esporte": ["Futebol", "Vôlei", "Tênis"], "Horas": [9.5, 5.5, 3.5]}
        )
        fig_pie = px.pie(
            df_tempo,
            names="Esporte",
...
def tela_admin():
    ...
    with col1:
        st.metric(
            label="💰 Lucro total (mês)", value="R$ 18.640", delta="+12% vs março"
        )
    with col2:
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Ocupação (%)": st.column_config.ProgressColumn(
                "Ocupação (%)", min_value=0, max_value=100, format="%d%%"
            ),
